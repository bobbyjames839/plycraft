...
import { AccordionItem } from '../components/productDetail/Accordion';
...
export function ProductDetail() {
    ...
    const specs: Array<{ key: string; title: string; content: string | undefined }> = [
        { key: 'size', title: d?.size?.title || 'Size', content: d?.size?.description },
        { key: 'style', title: d?.style?.title || 'Style', content: d?.style?.description },
        { key: 'materials', title: d?.materials?.title || 'Materials', content: d?.materials?.description },
        { key: 'colours', title: d?.colours?.title || 'Colours', content: d?.colours?.description },
    ].filter(s => !!s.content);
    ...
    return (
        <div className="pd-page">
                    ...
                    {isMobile ? (
                        <div className="pd-carousel" aria-roledescription="carousel">
                            <div className="pd-carousel-viewport">
                                <div className="pd-carousel-track" style={{ transform: `translateX(-${active * 100}%)` }}>
                                    {images.map((src, idx) => (
                                        <div className="pd-carousel-slide" role="group" aria-roledescription="slide" aria-label={`${idx + 1} of ${images.length}`} key={idx}>
                            ...
                            {images.length > 1 && (
                                <>
                                    ...
                                    <div className="pd-carousel-dots" role="tablist" aria-label="Choose product image">
                                        {images.map((_, i) => (
                                            <button key={i} role="tab" aria-selected={i === active} className={`pd-dot ${i === active ? 'is-active' : ''}`} onClick={() => setActive(i)} aria-label={`Show image ${i + 1}`} />
                                        ))}
                                    </div>
                                </>
                            )}
            ...
            <p className='pd-accordion-title'>Specifications</p>
            <div className="pd-accordion" aria-label="Product details" role="region">
                {specs.map((s, idx) => (
                    <AccordionItem
                        key={s.key}
                        index={idx}
